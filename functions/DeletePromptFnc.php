...
function DeletePromptStaffCert($title, $certificate_id, $action = 'delete') {
    ...
    if (!$_REQUEST['delete_ok'] && !$_REQUEST['delete_cancel']) {
        ...
        echo "<CENTER><h4>"._areYouSureYouWantTo." $action "._that." " . (strpos($title, ' ') === false ? ucwords($title) : $title) . "?</h4><br><INPUT type=submit class=\"btn btn-primary\" value="._ok." onclick='load_link(\"Modules.php?modname=$_REQUEST[modname]&include=CertificationInfoInc&custom=staff&category_id=4&delete_ok=1&certification_id=$certificate_id\");'>&nbsp;<INPUT type=button class='btn btn-primary' name=delete_cancel value="._cancel." onclick='load_link(\"Modules.php?modname=$_REQUEST[modname]&include=CertificationInfoInc&custom=staff&category_id=4\");'></CENTER>";
        PopTable('footer');
        return false;
    } else {
        return true;
...
//quick fix for new Staff Education Information - Degrees Earned. Need to modify function DeletePrompt later to make a one-size fits all DeletePrompt
function DeletePromptStaffDegree($title, $degree_id, $action = 'delete') {
    $tmp_REQUEST = $_REQUEST;
    ...
    if (!$_REQUEST['delete_ok'] && !$_REQUEST['delete_cancel']) {
        echo '<BR>';
        PopTable('header', _confirm . (strpos($action, ' ') === false ? ucwords($action) : $action));
        echo "<CENTER><h4>"._areYouSureYouWantTo." $action "._that." " . (strpos($title, ' ') === false ? ucwords($title) : $title) . "?</h4><br><INPUT type=submit class=\"btn btn-primary\" value="._ok." onclick='load_link(\"Modules.php?modname=$_REQUEST[modname]&include=$include&custom=staff&category_id=6&delete_ok=1&id=$degree_id\");'>&nbsp;<INPUT type=button class='btn btn-primary' name=delete_cancel value="._cancel." onclick='load_link(\"Modules.php?modname=$_REQUEST[modname]&include=$include&custom=staff&category_id=6\");'></CENTER>";
        PopTable('footer');
        return false;
    } else {
        return true;
