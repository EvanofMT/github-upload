...
function TextInput($value, $name, $title = '', $options = '', $div = true, $divOptions = '', $required='') {
    ...
    if (AllowEdit() && !$_REQUEST['_openSIS_PDF']) {
        ...
        if ((trim(str_replace("","",$value)) == '' || trim($div) == false))
            return (($title != '') ? '<label for="' . $name . '" class="control-label text-right col-lg-4">' . str_replace('*', '<span class="text-danger">*</span>', $original_title) . '</label><div class="col-lg-8">' : '') . "<INPUT class=\"form-control\" type=text id=$name name=$name " . (($value || $value === '0') ? "value=\"$value\"" : '') . " $options $required>" . (($title != '') ? '</div>' : '');
        else {
...
function CheckboxInputSwitch($value, $name, $title = '', $checked = '', $new = false, $yes = _yes, $no = _no, $extra = '', $switchery_color = 'switch-default', $size = 'sm') {
    // $checked has been deprecated -- it remains only as a placeholder, use VALUE option instead (a value of VALUE='Y' will cause switch to be enabled)
    if (Preferences('HIDDEN') != 'Y') {
        $div = false;
    }
...
//Create the option to make a require field
//ORIGINAL - function SelectInput($value, $name, $title = '', $options, $allow_na = 'N/A', $extra = '', $div = true) {
function SelectInput($value, $name, $title = '', $options, $allow_na = 'N/A', $extra = '', $div = true, $required = '') {
...
//        $return = (($title != '') ? '<label class="control-label text-right col-lg-4">' . $title . '</label><div class="col-lg-8">' : '');
        ...
        $return .= "<SELECT name=$name $extra class=form-control $required>";
...
//ADDED by Africa's Hope - Evan Stewart
//create a REquire parameter for DateInputAY
//ORIGINAL - function DateInputAY($value, $name, $counter = 1, $div_visibility = false) {
function DateInputAY($value, $name, $counter = 1, $div_visibility = false, $required ='') {
    //Added by Africa's Hope - Evan Stewart.  The $title variable below is not defined.  This might need to be added to the function parameters later. For now, define it as empty to avoid errors
    $title = "";
    ...
    if (AllowEdit() && !$_REQUEST['_openSIS_PDF']) {
        ...
        if ($value != '') {
            ...
            //ADDED by Africa's Hope - Evan Stewart
            //If you use MySQL SELECT command to format the date is local preference, then change the yearVal, monthVal, dayVal index positions below
            //ORIG - 
            /*
            ...
            if ($div_visibility == false) {
                ...
                return '<div id="date_div_' . $counter . '" class="fake_datepicker" onClick="$(\'#original_date_' . $counter . '\').show(); $(\'#date_div_' . $counter . '\').hide();">' . (($title != '') ? '<label class="control-label col-md-4 text-right">' . $title . '</label><div class="col-md-8">' : '') . '<div class="input-group"><span class="input-group-addon"><i class="icon-calendar22"></i></span><input type="text" readonly="readonly" data-calid="' . $counter . '" class="form-control" value="' . ProperDateAY($value) . '"'. $required .'/></div>' . (($title != '') ? '</div>' : '') . '</div>'
                      //. '<div id="date_div_' . $counter . '" class="fake_datepicker" onClick="$(\'#original_date_' . $counter . '\').show(); $(\'#date_div_' . $counter . '\').hide();" style="display: inline" >' . (($title != '') ? '<label class="control-label col-md-4 text-right">' . $title . '</label><div class="col-md-8">' : '') . '<div class="input-group"><span class="input-group-addon"><i class="icon-calendar22"></i></span><input type="text" readonly="readonly" data-calid="'.$counter.'" class="form-control" value="' .$value. '" /></div>' . (($title != '') ? '</div>' : '') . '</div>'
                        ...
                        //ADDED by Africa's Hope - Evan Stewart
                        //ORIGINAL - . '<input type="text" id="date_' . $counter . '" placeholder="' . $title . '" id="date_' . $counter . '" ' . $show . ' class="form-control daterange-single" value="' . $value . '" >'
                        . '<input type="text" id="date_' . $counter . '" placeholder="' . $title . '" id="date_' . $counter . '" ' . $show . ' class="form-control daterange-single" value="' . $value . '"' . $required .'>'
                        //END Africa's Hope addition
                        . '</div>'
                ...
                return '<div class="input-group datepicker-group" id="original_date_' . $counter . '" ' . $show . '>'
                        . '<span class="input-group-addon"><i class="icon-calendar22"></i></span>'
                        //ADDED by Africa's Hope - Evan Stewart
                        //ORIGINAL - . '<input type="text" placeholder="' . $title . '" id="date_' . $counter . '" ' . $show . ' class="form-control daterange-single" value="" >'
                        . '<input type="text" placeholder="' . $title . '" id="date_' . $counter . '" ' . $show . ' class="form-control daterange-single" value="" '.$required.'>'
                        //END Africa's Hope addition
                        . '</div>'
                        . '<input type=hidden ' . $monthVal . ' id="monthSelect_date_' . $counter . '" name="month_' . $name . '" >'
                        . '<input type=hidden ' . $dayVal . '  id="daySelect_date_' . $counter . '"   name="day_' . $name . '">'
