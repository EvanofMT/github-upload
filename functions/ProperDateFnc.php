...
function ProperDate($date='',$length='long')
{
	...
	if($date)
		//ORIGINAL - return date((($length=='long' || Preferences('MONTH')!='F')?Preferences('MONTH'):'M').$sep.Preferences('DAY').$sep.Preferences('YEAR'),mktime(0,0,0,$month,$day,$year));
		//Added by Africa's Hope - 2020-12-18 - Evan Stewart
		//To make dates (on Transcript) appear in Africa format DD-MM-YYYY
		return date(Preferences('DAY').$sep.(($length=='long' || Preferences('MONTH')!='F')?Preferences('MONTH'):'M').$sep.Preferences('YEAR'),mktime(0,0,0,$month,$day,$year));
}
...
function ProperDateAY($date='',$length='long'){   
	...
	$m['12']='Dec';
		...
		////If you use MySQL SELECT command to format the date is local preference, then change the year, month, day index positions below
        $year = $break_date[0];
		$month = $break_date[1];
		$day = $break_date[2];
		
		//ORIG - return mkSisDate($break_date[0],$break_date[1],$break_date[2]);
		return mkSisDate($year,$month,$day);
	}
}
...
//ADDED by Africa's Hope - Evan Stewart
//OASIS UI will instruct users to enter dates as DD-MM-YYYY
//change the order of received parameters so that it knows what to do
function mkSisDate($year,$month,$date){
	...
	//To make dates (on Transcript) appear in Africa format DD-MM-YYYY
	//Date Format Options
		//East Africa Date - DD/MM/YYYY -- return $dayFormat.'/'.$monthFormat.'/'.$yearFormat;
		//ORIGINAL - MM/DD/YYYY -- return $monthFormat.'/'.$dayFormat.'/'.$yearFormat;
		//MySQL format - YYYY-MM-DD -- return $yearFormat.'-'.$monthFormat.'-'.$dayFormat;
	return $yearFormat.'-'.$monthFormat.'-'.$dayFormat; //using MySQL format
