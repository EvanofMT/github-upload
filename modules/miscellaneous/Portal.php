...
$update_notify_s = DBGet(DBQuery('SELECT VALUE FROM program_config WHERE school_id=\'' . UserSchool() . '\'  AND program=\'UPDATENOTIFY\' AND title=\'display_school\' LIMIT 0, 1'));
        if ($update_notify_s[1]['VALUE'] == 'Y') {
            ...
            //use this so that setup reminders still display when installer uses Suggested Data.  This will help new users become familiar with OASIS after first install
            //Keep showing school setup until at least one semester is created
            $mp_setup_sem = DBGet(DBQuery('SELECT COUNT(*) as REC FROM school_semesters WHERE SCHOOL_ID=' . UserSchool() . ' AND SYEAR=' . UserSyear()));
...
//        }
            ...
            //ADDED by Afric'a Hope
            //Add semester check ($mp_setup_sem) to initial setup
            //ORIGINAL - if ($cal_setup[1]['REC'] == 0 || $mp_setup[1]['REC'] < 1 || $att_code_setup[1]['REC'] == 0 || $grade_scale_setup[1]['REC'] == 0 || $enroll_code_setup[1]['REC'] == 0 || $grade_level_setup[1]['REC'] == 0 || $periods_setup[1]['REC'] == 0 || $rooms_setup[1]['REC'] == 0) {
            if ($cal_setup[1]['REC'] == 0 || $mp_setup[1]['REC'] < 1 || $mp_setup_sem[1]['REC'] < 1 || $att_code_setup[1]['REC'] == 0 || $grade_scale_setup[1]['REC'] == 0 || $enroll_code_setup[1]['REC'] == 0 || $grade_level_setup[1]['REC'] == 0 || $periods_setup[1]['REC'] == 0 || $rooms_setup[1]['REC'] == 0) {
            //END Africa's Hope addition
                $width = 0;
                $percent = 0;
