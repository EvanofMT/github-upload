...
if ($_REQUEST['modfunc'] != 'remove') {
    ...
    if ($_REQUEST['table'] !== 'new' && $_REQUEST['modfunc'] != 'edit') {
        if (count($attendance_codes_RET) == 0) {
            ...
            echo "</div>";
            if ($_REQUEST['table'] != 0) {
                echo '<div class="pt-15">' . button("edit", _editCategoryTitle, "Modules.php?modname=$_REQUEST[modname]&modfunc=edit&table=$_REQUEST[table]") . '<br>';
                echo button("remove", _deleteThisCategory, "Modules.php?modname=$_REQUEST[modname]&modfunc=remove&table=$_REQUEST[table]") . '</div>';
            }
        ...
        $_openSIS['selected_tab'] = "Modules.php?modname=$_REQUEST[modname]&table=$_REQUEST[table]";
        echo PopTable('header', $tabs);
        echo '<CENTER>' . _newCategoryTitle . '<INPUT type=text id=new_category_title name=new_category_title value="' . $code_cat . '"></CENTER>';
