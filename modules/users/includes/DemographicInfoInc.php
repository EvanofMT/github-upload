...
if ($_REQUEST['staff_id'] == 'new') {
    ...
    //ADDED by AFrica's Hope - Evan Stewart //add required to First Name and Last Name function parameters.  Add * symbole to first and last name label
    echo '<div class=col-md-2><label>'._salutation.'</label>' . SelectInput($staff['TITLE'], 'staff[TITLE]', '', array('Mr.' => 'Mr.', 'Mrs.' => 'Mrs.', 'Ms.' => 'Ms.', 'Miss' => 'Miss', 'Dr' => 'Dr', 'Rev' => 'Rev'), '') . '</div><div class="col-md-3"><label>'._firstName.'<span class="text-danger">*</span></label>' . TextInput($staff['FIRST_NAME'], 'staff[FIRST_NAME]', '', 'maxlength=50 required') . '</div><div class="col-md-3"><label>'._middleName.'</label>' . TextInput($staff['MIDDLE_NAME'], 'staff[MIDDLE_NAME]', '', 'maxlength=50') . '</div><div class="col-md-3"><label>'._lastName.'<span class="text-danger">*</span></label>' . TextInput($staff['LAST_NAME'], 'staff[LAST_NAME]', '', 'maxlength=50 class=cell_floating required') . '</div><div class="col-md-1"><label>Suffix</label>' . SelectInput($staff['NAME_SUFFIX'], 'staff[NAME_SUFFIX]', '', array('Jr.' => 'Jr.', 'Sr.' => 'Sr.', 'II' => 'II', 'III' => 'III', 'IV' => 'IV', 'V' => 'V'), '', '') . '</div>';
    echo '</div>'; //.form-group    
} else {
...
echo DateInputAY($staff['BIRTHDATE'] != "" && $staff['BIRTHDATE'] != "0000-00-00" ? $staff['BIRTHDATE'] : "", 'staff[BIRTHDATE]', 1) . '</div><p class="dateTip">'._dateFormatTip.'</p></div>';
echo '</div>'; //.col-md-4
echo '</div>'; //.row
...
echo '<div class="row">';
echo '<div class="col-md-6">';
//ADDED by Africa's Hope - Evan Stewart // add 'required' to text input parameters for Email
echo '<div class="form-group"><label class="control-label text-right col-lg-4">'._email.' <span class="text-danger">*</span></label><div class="col-lg-8">' . TextInput($staff['EMAIL'], 'staff[EMAIL]', '', 'autocomplete=off id=email_id class=cell_medium required onkeyup=check_email(this,' . $id_sent . ',2); onblur=check_email(this,' . $id_sent . ',2) ') . '<p id="email_error" class="help-block"></p></div></div>';
echo '</div>'; //.ocl-md-6
echo '<div class="col-md-6">';
