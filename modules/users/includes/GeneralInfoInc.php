...
if ($_REQUEST['profile'] != 'none') {
    ...
    if (!isset($staff['STAFF_ID'])) {
        ...
        //ADDED By Africa's Hope - Evan Stewart - 07-30-2021 - remove password validator check.  The validator forces all user passwords to be unique.  I want to force unique password per user 
            //ORIGINAL - echo TextInput(str_repeat('*', strlen($staff['PASSWORD'])), 'login_authentication[PASSWORD]', '', "size=25 maxlength=100 class=cell_floating AUTOCOMPLETE = off onkeyup=passwordStrength(this.value);validate_password(this.value);");
        echo TextInput(str_repeat('*', strlen($staff['PASSWORD'])), 'login_authentication[PASSWORD]', '', "size=25 maxlength=100 class=cell_floating AUTOCOMPLETE = off onkeyup=passwordStrength(this.value);");
        //END Africa's Hope addition

    } else {
        //for existing users while updating
        //ADDED By Africa's Hope - Evan Stewart - 07-30-2021 - remove password validator check.  The validator forces all user passwords to be unique.  I want to force unique password per user 
            //ORIGINAL - echo TextInput(str_repeat('*', strlen($staff['PASSWORD'])), 'login_authentication[PASSWORD]', '', "size=25 maxlength=100 class=cell_floating AUTOCOMPLETE = off onkeyup=passwordStrength(this.value);validate_password(this.value,$staff[STAFF_ID]);");
        echo TextInput(str_repeat('*', strlen($staff['PASSWORD'])), 'login_authentication[PASSWORD]', '', "size=25 maxlength=100 class=cell_floating AUTOCOMPLETE = off onkeyup=passwordStrength(this.value);");
        //END Africa's Hope addition
    }
    ...
    echo '<div class="form-group">';
    //ADDED By Africa's Hope - Evan Stewart - 07-30-2021 - remove password validator check.  The validator forces all user passwords to be unique.  I want to force unique password per user 
    //ORIGINAL - echo TextInput('', 'FRESH_PASSWORD', 'Password', "size=25 maxlength=100 AUTOCOMPLETE = off onkeyup=passwordStrength(this.value);validate_password(this.value);");
    echo TextInput('', 'FRESH_PASSWORD', 'Password', "size=25 maxlength=100 AUTOCOMPLETE = off onkeyup=passwordStrength(this.value);");
    //END Africa's Hope addition
