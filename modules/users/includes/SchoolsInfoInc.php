...
if ($_REQUEST['teacher_view'] != 'y') {
    ...
    foreach ($school_admin as $index => $school) {
        if ($_REQUEST['day_values']['START_DATE'][$school['SCHOOL_ID']]) {
            ...
            //$check_start_date = $_REQUEST['year_values']['START_DATE'][$school['SCHOOL_ID']] . '-' . MonthFormatter(strtoupper($_REQUEST['month_values']['START_DATE'][$school['SCHOOL_ID']]), 1) . '-' . $_REQUEST['day_values']['START_DATE'][$school['SCHOOL_ID']];
...
if (!$_REQUEST['modfunc']) {
    ...
    if (isset($_REQUEST['school_info_id'])) {
        ...
        if ($_REQUEST['school_info_id'] != '0' && $_REQUEST['school_info_id'] !== 'old') {
            ...
            //ADDED by Africa's Hope - Evan Stewart //add 'required' text to SelectInput function parameter
            ...
            echo '<div class="row">';
            echo '<div class="col-md-6">';
            //ADDED by Africa's Hope - Evan Stewart //add date format tip below date input field
            echo '<div class="form-group"><label class="control-label text-right col-lg-4">'._joiningDate.' <span class=text-danger>*</span></label><div class="col-lg-8">' . DateInputAY(isset($this_school['JOINING_DATE']) && $this_school['JOINING_DATE']!="" ? $this_school['JOINING_DATE'] : "", 'values[JOINING_DATE]', 1, 'class=cell_medium') . '</div><p class="dateTip">'._dateFormatTip.'</p></div>';
            echo '<input type=hidden id=end_date_school value="' . $get_end_date . '" >';
            echo '</div><div class="col-md-6">';
            //ADDED by Africa's Hope - Evan Stewart //add date format tip below date input field
            echo '<div class="form-group"><label class="control-label text-right col-lg-4">'._endDate.'</label><div class="col-lg-8">' . DateInputAY($this_school['END_DATE']!="" ? $this_school['END_DATE'] : "", 'values[ENDING_DATE]', 2, '') . '</div><p class="dateTip">'._dateFormatTip.'</p></div>';
...
//            echo '</div>'; //.row
            ...
            if (!$this_school_mod['PASSWORD']) {
                //ADDED By Africa's Hope - Evan Stewart - 07-30-2021 - remove password validator check.  The validator forces all user passwords to be unique.  I want to force unique password per user 
                    //ORIGINAL - echo TextInputModHidden('', 'PASSWORD', '', 'size=20 maxlength=100 AUTOCOMPLETE = off onblur=passwordStrength(this.value);validate_password_staff(this.value);');
                echo TextInputModHidden('', 'PASSWORD', '', 'size=20 maxlength=100 AUTOCOMPLETE = off onblur=passwordStrength(this.value);');
                //END Africa's Hope addition

                echo '<span id="ajax_output_st"></span>';
            } else {
                //ADDED By Africa's Hope - Evan Stewart - 07-30-2021 - remove password validator check.  The validator forces all user passwords to be unique.  I want to force unique password per user 
                    //ORIGINAL - echo TextInputModHidden(array($this_school_mod['PASSWORD'], str_repeat('*', strlen($this_school_mod['PASSWORD']))), 'staff_school[PASSWORD]', '', 'size=20 maxlength=100 AUTOCOMPLETE = off onkeyup=passwordStrength(this.value);validate_password(this.value);');
                echo TextInputModHidden(array($this_school_mod['PASSWORD'], str_repeat('*', strlen($this_school_mod['PASSWORD']))), 'staff_school[PASSWORD]', '', 'size=20 maxlength=100 AUTOCOMPLETE = off onkeyup=passwordStrength(this.value);');
                //END Africa's Hope addition
            }
...
//                $columns = array('SCHOOL_ID' => '<a><INPUT type=checkbox value=Y name=controller onclick="checkAll(this.form,this.form.controller.checked,\'unused\');" /></a>', 'TITLE' => 'School', 'PROFILE' => 'Profile', 'START_DATE' => 'Start Date', 'END_DATE' => 'Drop Date', 'ID' => 'Status');
                ...
                $columns = array('SCHOOL_ID' => '<a><INPUT type=checkbox value=Y name=controller onclick="checkAllDtMod(this,\'values[SCHOOLS]\');" /></a>',
                 'TITLE' =>_school,
                 'PROFILE' =>_profile,
                 'START_DATE' =>_startDate.' <span class="dateTip">'._dateFormatTip.'</span>',
                 'END_DATE' =>_dropDate.' <span class="dateTip">'._dateFormatTip.'</span>',
                 'ID' =>_status,
                );
...
function _makeStatus($value, $column) {
    ...
    if ($_REQUEST['staff_id'] == 'new')
        ...
        if ($dates[1]['START_DATE'] != '0000-00-00') {
            $sql = 'SELECT SCHOOL_ID FROM staff s,staff_school_relationship ssr WHERE ssr.STAFF_ID=s.STAFF_ID AND ssr.SCHOOL_ID=' . $THIS_RET['SCHOOL_ID'] . ' AND ssr.STAFF_ID=' . $_SESSION['staff_selected'] . ' AND ssr.SYEAR=(SELECT MAX(SYEAR) FROM  staff_school_relationship WHERE SCHOOL_ID=' . $THIS_RET['SCHOOL_ID'] . ' AND STAFF_ID=' . $_SESSION['staff_selected'] . ')  AND (ssr.END_DATE>=\'' . date('Y-m-d') . '\' OR ssr.END_DATE IS NULL OR ssr.END_DATE=\'0000-00-00\')';
        }
        $user_exist_school = DBGet(DBQuery($sql));
        if (!empty($user_exist_school))
            $status_value = 'Active';
