...
if (!$_REQUEST['include']) {
    ...
    elseif ($_REQUEST['include'] == 'EducationInfoInc')
         ...
         $sql .= '(' . substr($fields, 0, -1) . ') values(' . substr($values, 0, -1) . ')';
            ...
            if ($error != true) {
                ...
                if ($school_array) {
                    ...
                    //ADDED by Africa's Hope - Evan Stewart
                    //This is where the Teacher Start Date is set when the account is first created.
                    //When new staff is created, make the School Start Date is set to the date of accoutn creation
                    //This will cause confusion, but is there a better way allow the admin to enter a start date on account creation?
                    //END Africa's Hope addition
...
if ($_REQUEST['modfunc'] == 'delete' && basename($_SERVER['PHP_SELF']) != 'index.php' && AllowEdit()) {
    ...
    /*if (DeletePrompt('certification NOway')) {
    ...
    */
    ...
    //ADDED by Africa's Hope - Evan Stewart
    # ------------------------------------  For Education - Degrees Earned Start ------------------------------------------- #
    /*if ($_REQUEST['category_id'] == 6){
        $sql = "DELETE FROM staff_degrees_earned WHERE ID='".$_REQUEST['DEGREE_ID']."'";
        if (DeletePrompt($sql)) {
            
            echo "<p> THE Delete SQL is: $sql</p>";
            DBQuery("DELETE FROM staff_degrees_earned WHERE DEGREE_ID='".$_REQUEST['DEGREE_ID'] ."'");

            unset($_REQUEST['modfunc']);

            $_REQUEST['degree_id'] = 'new';
        }
    }*/
    //END AFrica's Hope addition
}
...
if ($_REQUEST['values'] && $_REQUEST['include'] == 'EducationInfoInc'){
    $deg_table = "staff_degrees_earned";
    SaveData(array($deg_table=> "ID='__ID__'", 'fields' => array($deg_table => 'STAFF_ID,'),'values' => array($deg_table => "'" . UserStaffID() . "',")));
}
//END Africa's Hope addition
        ...
        foreach ($categories_RET as $category) {
            ...
            if ($can_use_RET['users/Staff.php&category_id=' . $category['ID']]) {
                ...
                elseif ($category['ID'] == '5')
                    $include = 'ScheduleInc';
                //ADDED BY Africa's Hope - Evan Stewart
                //create tab for Staff Degrees earned
                elseif ($category['ID'] == '6')
                    $include = 'EducationInfoInc';
                //END Africa's Hope addition
                ...
                switch ($category['TITLE'] ) {
                    ...
                    case 'Education Information':
                        $categoryTitle = _educationInformation;
                        break;
                    //END Africa's Hope addition
                    default:
...
//ADDED by Africa's Hope - Evan Stewart
//add condition to this line, only show if Category ID 4 is active, because this is CertificationInfo category id
//ORIG - if ($_REQUEST['modfunc'] == 'remove'){
if ($_REQUEST['modfunc'] == 'remove' AND $_REQUEST['category_id'] == 4) {
...
//ADDED by Africa's Hope - Evan Stewart
//need this to send delete/remove request back to EducationInfo script
//this activates if the delete button [X] is clicked on the Educatoninfo page
if ($_REQUEST['modfunc'] == 'remove' AND $_REQUEST['category_id'] == 6) {
    include('modules/users/includes/'.$_REQUEST['include'].'.php');
}
//END Africa's Hope addition
