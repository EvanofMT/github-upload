...
//for new Degrees Earned and Credits Transferred feature to show up on Transcript
//Degrees Earned
$degquery = "SELECT degree_type, field_of_study, earned_date, school_name, school_location FROM student_degrees_earned WHERE student_id = ";
//Credit Transferred
$crdtrnquery = "SELECT sum(credit_earned) AS credits FROM student_report_card_grades rcg 
INNER JOIN history_marking_periods hmp 
ON rcg.marking_period_id = hmp.marking_period_id
WHERE rcg.student_id=";
//End edit by Africa's Hope
...
if ($_REQUEST['modfunc'] == 'save') {
    ...
    foreach ($_REQUEST['st_arr'] as $arrkey => $student_id) {
        ...
        if (User('PROFILE') == 'admin' || UserStudentID() == $student_id) {
            ...
            //Added by Africa's Hope - Evan Stewart
            //for new Degrees Earned and Credits Transferred feature to show up on Transcript
            //Check for Credit Transferred
            $CTRET = DBGet(DBQuery($crdtrnquery . $student_id)); 
            //Check for Degrees Earned
            $DRET = DBGet(DBQuery($degquery . $student_id),array('EARNED_DATE' => 'ProperDate'));
			//END edits by Africa's Hope
            ...
            foreach ($tcolumns as $tcolumn) {
                foreach ($tcolumn as $tsection) {
                    ...
                    foreach ($tsection as $trec) {
                        ...
                        if ($trec['COURSE_PERIOD_ID'] != '') {
                            ...
                            if ($grd_scl[1]['GRADE_SCALE_ID'] != '') {
                                ...
                                $evanSQL = 'SELECT GPA_CAL FROM report_card_grade_scales WHERE ID=' . $grd_scl[1]['GRADE_SCALE_ID'];
                        ...
                        //ADDED by Africa's Hope - Evan Stewart
                        if ($get_mp_tp[1]['MP_TYPE'] == 'trimester' && $get_mp_tp[1]['MP_TYPE'] == $get_mp_tp_m[1]['MP_TYPE']) {
                            $total_QP_value_tr += $QP_value;
                            $cred_earned_tr += $trec['CREDIT_EARNED'];
                        }
                        //END Africa's Hope addition
                    ...
                    //ADDED by Africa's Hope - Evan Stewart
                    $total_QP_transcript_tr = $total_QP_transcript_tr + $total_QP_value_tr;
                    $total_CGPA_earned_tr = $total_CGPA_earned_tr + $cred_earned_tr;
                    //END Africa's Hope addition
                ...
                <div class="transcript-header m-t-10 m-b-20">
                    ...
                    <div class="transcript-student-overview">
                        <table class="table">
                            ...
                            <tr>
                                <td class="p-r-30"><?=_totalCreditTransferred  //Added by Africa's Hope - Evan Stewart?></td>
                                <?php 
                                foreach ($CTRET as $rec){
                                    if ($rec['CREDITS'] <= 0){
                                        echo "<td>0</td>";
                                    } else {
                                        echo "<td>". sprintf("%01.2f",$rec['CREDITS'])."</td>";
                                    }
                                }/* //End Africa's Hope addition*/
                                ?>
                            </tr>
                    ...
                    <div class="transcript-degrees-earned">
                        <?php if ($DRET): /* //Added by Africa's Hope - Evan Stewart - for new Degrees Earned feature*/ ?>
                        <h4 class="f-s-15 m-b-0 m-t-0 text-blue"><?=_degreesEarned?></h4>
                        <table class="table">
                            <th><?=_degree?></th>
                            <th><?=_fieldOfStudy?></th>
                            <th><?=_date?></th>
                            <th><?=_schoolName?></th>
                            <th><?=_schoolLocation?></th>
                            <?php
                            foreach ($DRET as $rec){
                                echo "<tr>
                                <td>{$rec['DEGREE_TYPE']}</td>
                                <td>{$rec['FIELD_OF_STUDY']}</td>
                                <td>{$rec['EARNED_DATE']}</td>
                                <td>{$rec['SCHOOL_NAME']}</td>
                                <td>{$rec['SCHOOL_LOCATION']}</td>
                                </tr>";
                            }
                            ?>
                        </table>
                        <?php endif /* //End Africa's Hope addition*/ ?>
                    </div>
                </div>
                    ...
                    <table class="invoice-table table-bordered">
                        <thead>                            
                            <tr>
                                ...
                                <?php //Added by Africa's Hope - 2020-12-18 - Evan Stewart
                                //Africa's Schools don't use wieghted GP.  Uncomment to show and also enable editing of Weighted GP on the /modules/grades/ReportCardGrades.php file
                                ?>
