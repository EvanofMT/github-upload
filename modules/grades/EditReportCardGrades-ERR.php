...
/*ADDED by Africa's Hope - Evan Stewart - 05-20-2021 - commenting out RedirectModulesInc.php.  The file does not exist and the closest file 'RedirectModules.php' has already been included up the chain */
...
if (UserStudentID()) {
    ...
    $tab_id = (isset($_REQUEST['tab_id']) && $_REQUEST['tab_id'] ? $_REQUEST['tab_id'] : 'grades');
 ...
 //            print_r($_REQUEST);
    ...
    if (!$_REQUEST['modfunc']) {
       ...
       AND mp.school_id = \'' . UserSchool() . '\' order by mp.post_end_date';
        ...
        $last_posted = null; //was set to _null
        ...
        if ($mp_id != "0") {
            ...
            /*Added by Africa's Hope - Evan Stewart - 05-20-2021 - change Course_Title input to select dropdown showing courses from Course title */
            ...
            if (isset($_REQUEST['tab_id']) && $_REQUEST['tab_id'] == 'grades' || $_REQUEST['tab_id'] == '') {
                ...
                //$functions = array('COURSE_TITLE' => 'makeSelectInputCourses',
                ...
                //ORIGINAL - $link['add']['html'] = array('COURSE_TITLE' => makeTextInput('', 'COURSE_TITLE'),
                ////// Example of SelectInput: SelectInput($student['GRADE_ID'], 'values[student_enrollment][' . $student_id . '][GRADE_ID]', '', $options, '', '','','REQUIRED')
                $link['add']['html'] = array('COURSE_TITLE' => makeSelectInputCourses('','COURSE_TITLE',$options),
         ...
         ListOutput($LO_ret, $LO_columns, '', '', $link, array(), $options);
            //PopTable('footer');
        }
...
function makeTextInput($value, $name) {
    ...
    if ($name == 'COURSE_TITLE')
        $extra = 'size=25 maxlength=100 class=form-control'; /* ADDED by Africa's Hope - Evan Stewart - 5-20-2021 - change maxlenth of COURSE_TITLE to 100 characters*/
