...
#***************************************************************************************
#
# This file was modified by Africa's Hope modification of OpenSIS 7.6
    ...
    if (count($_REQUEST['st_arr'])) {
        ...
        if (count($RET)) {
            ...
            foreach ($RET as $student) {
                ...
                $sql = DBGet(DBQuery('SELECT s.gender AS GENDER, etn.ethnicity_name AS ETHNICITY, s.ethnicity_id, s.common_name AS COMMON_NAME,  s.social_security AS SOCIAL_SEC_NO, s.birthdate AS BIRTHDAY, s.email AS EMAIL, s.phone AS PHONE, lan.language_name AS LANGUAGE, s.language_id, se.START_DATE AS START_DATE,sec.TITLE AS STATUS, se.NEXT_SCHOOL AS ROLLING  FROM students s, student_enrollment se,student_enrollment_codes sec, ethnicity etn, language lan WHERE s.STUDENT_ID=\'' . $_SESSION['student_id'] . '\'  AND se.SCHOOL_ID=\'' . UserSchool() . '\' AND se.SYEAR=sec.SYEAR AND s.STUDENT_ID=se.STUDENT_ID AND s.ethnicity_id=etn.ethnicity_id AND s.language_id=lan.language_id'), array('BIRTHDAY' => 'ProperDate'));
...
# ---------------------------------- Nurse Visit Record ---------------- #
                ...
                if ($_REQUEST['category']['5']) {
                    ...
                    if ($res_goal) {
                        echo "<table width='100%'><tr><td colspan=5 style=\"border-bottom:1px solid #333;  font-size:14px;  font-weight:bold;\">"._goals."</td></tr>
				<tr><td colspan=5 style=\"height:5px;\"></td></tr>";
                        ...
                        echo "<tr style='font-weight:bold;'><td>"._title.'</td><td>'._startDate.'</td><td>'._endDate.'</td><td>'._description.'</td></tr>';
                ...
                //Degrees Earned
                echo "<tr><td valign=top colspan=3>";
                if ($_REQUEST['category']['8']) {
                    $res_degrees = DBGet(DBQuery("SELECT degree_type, field_of_study, earned_date, school_name, school_location FROM student_degrees_earned WHERE student_id='" . $_SESSION['student_id'] . "'"), array('EARNED_DATE' => 'ProperDate'));
                    if ($res_degrees) {
                        echo "<table width='100%'><tr><td colspan=5 style=\"border-bottom:1px solid #333;  font-size:14px;  font-weight:bold;\">"._degreesEarned."</td></tr>";
                        echo "<tr style='font-weight:bold;'><td>"._degree."</td><td>"._fieldOfStudy."</td><td>"._date."</td><td>"._schoolName."</td><td>"._schoolLocation."</td></tr>";
                        foreach ($res_degrees as $row_degrees) {
                            echo '<tr><td>' . $row_degrees['TYPE'] . '</td><td>' . $row_degrees['FIELD_OF_STUDY'] . '</td><td>' . $row_degrees['EARNED_DATE'] . '</td><td>' . $row_degrees['SCHOOL_NAME'] . '</td><td>' . $row_degrees['SCHOOL_LOCATION'] . '</td></tr>';
                        }
                        echo '</table>';
                    }
                    echo "</td></tr>";
                ...
                //Credits Transferred
                echo "<tr><td valign=top colspan=3>";
                if ($_REQUEST['category']['9']) {
                    //$crdtrnquery = "SELECT sum(credit_hours) AS credits FROM student_credit_transferred WHERE student_id = ";
                    $res_crdtrans = DBGet(DBQuery("SELECT sum(credit_hours) AS credits FROM student_credit_transferred WHERE student_id='" . $_SESSION['student_id'] . "'"));
                    if ($res_crdtrans) {
                        echo "<table width='100%'><tr><td colspan=2 style=\"border-bottom:1px solid #333;  font-size:14px;  font-weight:bold;\">"._creditsTransferred."</td></tr>";
                        echo "<tr><td style='font-weight:bold;' width='21%'>"._creditHours.":</td>";
                        foreach ($res_crdtrans as $row_crdtrans) {
                            echo '<td>' . $row_crdtrans['CREDITS'] . '</td></tr>';
                        }
                        echo '</table>';
                    }
                    echo "</td></tr>";
