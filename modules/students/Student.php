...
/////////////////////////////////////////////////////////////////////////////////
        ...
        //Added by Africa's Hope - 2020-12-15 - Evan Stewart
        //for new Degrees Earned and Credits Transferred feature
        elseif ($_REQUEST['include'] == 'CreditTransferredInc')
            $_REQUEST['category_id'] = '8';
        elseif ($_REQUEST['include'] == 'DegreesEarnedInc')
...
//          $filename =  $upload->target_path;
                ...
                //To handle updates to Degrees Earned and Credits Transferred feature
                if ($_REQUEST['values'] && $_REQUEST['include'] == 'DegreesEarnedInc'){
                    $deg_table = "student_degrees_earned";
                    SaveData(array($deg_table=> "ID='__ID__'", 'fields' => array($deg_table => 'STUDENT_ID,'),'values' => array($deg_table => "'" . UserStudentID() . "',")));
                }

                if ($_REQUEST['values'] && $_REQUEST['include'] == 'CreditTransferredInc'){
                    $ct_table = "student_credit_transferred";
                    SaveData(array($ct_table=> "ID='__ID__'", 'fields' => array($ct_table => 'STUDENT_ID,'),'values' => array($ct_table => "'" . UserStudentID() . "',")));
                }

                //Modified by Africa's Hope - Evan Stewart
                //Needed to change original code so that Degrees Earned and Credits Transferred won't load twice when updates applied 
                ///ORIGINAL by OpenSIS -- if ($_REQUEST['include'] != 'GeneralInfoInc' && $_REQUEST['include'] != 'AddressInc' && $_REQUEST['include'] != 'MedicalInc' && $_REQUEST['include'] != 'GoalInc' && $_REQUEST['include'] != 'OtherInfoInc' && $_REQUEST['include'] != 'EnrollmentInfoInc' && $_REQUEST['include'] != 'FilesInc')
                if ($_REQUEST['include'] != 'GeneralInfoInc' && $_REQUEST['include'] != 'AddressInc' && $_REQUEST['include'] != 'MedicalInc' && $_REQUEST['include'] != 'GoalInc' && $_REQUEST['include'] != 'OtherInfoInc' && $_REQUEST['include'] != 'EnrollmentInfoInc' && $_REQUEST['include'] != 'FilesInc' && $_REQUEST['include'] != 'DegreesEarnedInc' && $_REQUEST['include'] != 'CreditTransferredInc')
                    ...
                    foreach ($categories_RET as $category) {
                        if ($can_use_RET['students/Student.php&category_id=' . $category['ID']]) {
                            ...
                            elseif ($category['ID'] == '8')
                                $include = 'CreditTransferredInc';
                            elseif ($category['ID'] == '9')
                                $include = 'DegreesEarnedInc';
                            ...
                            switch ($category['TITLE'] ) {
                                ...
                                case 'Credit Transferred':
                                    $categoryTitle = _creditTransferred;
                                    break;
                                case 'Degrees Earned':
                                    $categoryTitle = _degreesEarned;
                                    break;
                                //END Africa's Hope addition
                                default:
