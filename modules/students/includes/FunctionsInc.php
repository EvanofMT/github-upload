...
//Added by AFrica's Hope - 2020-12-15 - by Evan Stewart
/// If a drop down menu is wanted for Degree Type on the /modules/students/includes/DegreesEarnedInc.php
function _makeTypeD($value, $column) {
    global $THIS_RET;
    ...
    if ($value != '---'){
        if ($value != ''){
            return SelectInput($value, 'values[student_degrees_earned][' . $THIS_RET['ID'] . '][TYPE]', '', array('PhD' => 'PhD', 'ThD' => 'ThD', 'MDiv' => 'MDiv', '---' => '---', $value => $value));
        }else{
            return SelectInput($value, 'values[student_degrees_earned][' . $THIS_RET['ID'] . '][TYPE]', '', array('PhD' => 'PhD', 'ThD' => 'ThD', 'MDiv' => 'MDiv','---' => '---'));
        }
    } else {
        return TextInput($value, 'values[student_degrees_earned][' . $THIS_RET['ID'] . '][TYPE]');
    }
}
