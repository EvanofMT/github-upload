...
if (!$_REQUEST['modfunc']) {
    ...
    //ADDED by Africa's Hope - Evan Stewart //add date format tip to date column headers
    $columns = array('DOCTORS_NOTE_DATE' =>_date.' <span class="dateTip">'._dateFormatTip.'</span>',
    ...
    $columns = array('TYPE' =>_type,
     'MEDICAL_DATE' =>_date.' <span class="dateTip">'._dateFormatTip.'</span>',
     'COMMENTS' =>_comments,
    );
    foreach ($med_RET as $mi => $md) {
    ...
    if (count($med_RET) == 0)
        ...
        $plural = _immunizationOrPhysical;
    ...
    //ADDED by Africa's Hope - Evan Stewart //add date format tip to date column headers
    $columns = array('ALERT_DATE' =>_alertDate.' <span class="dateTip">'._dateFormatTip.'</span>',
     'TITLE' =>_medicalAlert,
    );
    foreach ($med_RET as $mi => $md) {
        $counter_for_date = $counter_for_date + 1;
    ...
    //ADDED by Africa's Hope - Evan Stewart //add date format tip to date column headers
    $columns = array('SCHOOL_DATE' =>_date.' <span class="dateTip">'._dateFormatTip.'</span>',
     'TIME_IN' =>_timeIn,
     'TIME_OUT' =>_timeOut,
     'REASON' =>_reason,
     'RESULT' =>_result,
