...
if ((!clean_param($_REQUEST['modfunc'], PARAM_NOTAGS) || clean_param($_REQUEST['modfunc'], PARAM_NOTAGS) == 'choose_course') && !clean_param($_REQUEST['course_modfunc'], PARAM_NOTAGS)) {
    ...
    if (clean_param($_REQUEST['modfunc'], PARAM_ALPHAMOD) != 'choose_course') {
        ...
        if ($_REQUEST['progress_id']) {
            ...
            //ADDED by Africa's Hope - Evan Stewart //add date format tip below date input field
            $header .= '<div class="col-md-6"><div class="form-group"><label class="control-label col-lg-4 text-right">'._endDate.'</label><div class="col-lg-8">' . DateInputAY($RET['END_DATE']!="" ? $RET['END_DATE'] : "", 'tables[' . $_REQUEST['goal_id'] . '][END_DATE]', 3) . '</div><p class="dateTip">'._dateFormatTip.'</p></div></div>'; // DateInput is copied from schoolsetup/MarkingPeriods.php line 296
            $header .= '</div>'; //.row
