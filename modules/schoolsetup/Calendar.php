...
if ((clean_param($_REQUEST['modfunc'], PARAM_ALPHAMOD) == 'create' || $cal_found_qr[1]['TOT'] == 0) && User('PROFILE') == 'admin') {
    ...
    $message .= '<div class="col-md-4">';
    $message .= '<div class="form-group">';
    //ADDED by Africa's Hope- Evan Stewart // add _dateFormatTip text to label
    $message .= '<label class="col-md-4 control-label text-right">'._from.' <span class="dateTip">'._dateFormatTip.'</span></label>';
    //END Africa's Hope addition
    $message .= '<div class="col-md-8">' . DateInputAY($fy_RET['START_DATE'], '_min', 1,'','required') . '</div>';
    $message .= '</div>'; //.form-group
    $message .= '</div>'; //.col-md-4
    $message .= '<div class="col-md-4">';
    $message .= '<div class="form-group">';
    //ADDED by Africa's Hope- Evan Stewart // add _dateFormatTip text to label
    $message .= '<label class="col-md-4 control-label text-right">'._to.' <span class="dateTip">'._dateFormatTip.'</span></label>';
    //END Africa's Hope addition
    $message .= '<div class="col-md-8">' . DateInputAY($fy_RET['END_DATE'], '_max', 2,'','required') . '</div>';
    $message .= '</div>'; //.form-group
    $message .= '</div>'; //.col-md-4
    ...
    $message .= '<div class="col-md-8"><div class="checkbox"><label><INPUT class="styled" type=checkbox value=Y name=weekdays[0] CHECKED>'._sunday.'</label></div> <div class="checkbox"><label><INPUT class="styled" type=checkbox value=Y name=weekdays[1] CHECKED>'._monday.'</label></div> <div class="checkbox"><label><INPUT class="styled" type=checkbox value=Y name=weekdays[2] CHECKED>'._tuesday.'</label></div> <div class="checkbox"><label><INPUT class="styled" type=checkbox value=Y name=weekdays[3] CHECKED>'._wednesday.'</label></div> <div class="checkbox"><label><INPUT class="styled" type=checkbox value=Y name=weekdays[4] CHECKED>'._thursday.'</label></div> <div class="checkbox"><label><INPUT class="styled" type=checkbox value=Y name=weekdays[5] CHECKED>'._friday.'</label></div> <div class="checkbox"><label><INPUT class="styled" type=checkbox value=Y name=weekdays[6] CHECKED>'._saturday.'</label></div></div>';
    //END Africa's Hope addition
    $message .= '</div>'; //.form-group
    $message .= '</div>'; //.col-md-4
    $message .= '<div class="col-md-4">';
...
if (clean_param($_REQUEST['modfunc'], PARAM_ALPHAMOD) == 'edit_calendar') {
    ...
    $message .= '<label class="col-md-4 control-label text-right">'._from.' <span class="dateTip">'._dateFormatTip.'</span></label>';
    //END Africa's Hope addition
    $message .= '<div class="col-md-8">' . DateInputAY($ac_RET['START_DATE'], '_min', 1,'','REQUIRED') . '</div>';
    $message .= '</div>'; //.form-group
    $message .= '</div>'; //.col-md-4
    $message .= '<div class="col-md-4">';
    $message .= '<div class="form-group">';
    ...
    $message .= '<label class="col-md-4 control-label text-right">'._to.' <span class="dateTip">'._dateFormatTip.'</span></label>';
    //END Africa's Hope addition
    $message .= '<div class="col-md-8">' . DateInputAY($ac_RET['END_DATE'], '_max', 2,'','REQUIRED') . '</div>';
    $message .= '</div>'; //.form-group
    $message .= '</div>'; //.col-md-4
    $message .= '</div>'; //.row 
    ...
    $message .= '<label class="col-md-4 control-label text-right">'._weekdays.'</label>';
    //ADDED by Africa's Hope - Evan Stewart
    //check all week days by default
...
function calendarEventsVisibility() {
    ...
    //Added by AFrica's Hope - Evan Stewart
    //hide Parent's profile from check box list
    //$profiles_RET = DBGet(DBQuery('SELECT ID,TITLE FROM user_profiles ORDER BY ID'));
    $return = ""; //define the return variable, wasn't defined before using it below
    $profiles_RET = DBGet(DBQuery('SELECT ID,TITLE FROM user_profiles WHERE TITLE != \'Parent\' ORDER BY ID'));
    //END Africa's Hope addition
    ...
    //Added by AFrica's Hope - Evan Stewart
    //remove reference to parent id.  Parent ID not used in OASIS
    ...
    foreach (array('admin' => ''._administratorWCustom.'', 'teacher' => ''._administratorWCustom.'') as $profile_id => $profile)
    //END Africa's Hope addition
    ...
    foreach ($profiles_RET as $profile) {
		...
		if($profile[TITLE] == "Super Administrator")
                    ...
                    //Parents aren't used in OASIS, so hide this control from Calendar setup
