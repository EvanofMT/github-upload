    ...
    case 'md':
        ...
        if ($_REQUEST['sch_type'] == 'variable') {
            ...
            foreach ($days as $day => $short_day) {
                ...
                //change the checkbox label abbreivations from signle letter ($day) to three letter ($short_day)
                ...
                $header .= '<td><p class="p-0 m-0">'.$short_day.'</p>' . CheckboxInput($value, 'tables[course_period_var][' . $course_period_id . '][DAYS][' . $day . ']', '', $checked, true, '', '', true) . '</td>';
...
function conv_day($short_date) {
    //ADDED by Africa's Hope - for language translation. this is the short names on the course period creation
    //ORIG -- $days = array('U' => 'Sun', 'M' => 'Mon', 'T' => 'Tue', 'W' => 'Wed', 'H' => 'Thu', 'F' => 'Fri', 'S' => 'Sat');
       
    $days = array('U' => _sun, 'M' => _mon, 'T' => _tue, 'W' => _wed, 'H' => _thu, 'F' => _fri, 'S' => _sat);
    //END Africa's Hope addition
