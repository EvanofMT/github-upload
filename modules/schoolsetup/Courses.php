...
if (clean_param($_REQUEST['course_modfunc'], PARAM_ALPHAMOD) == 'search') {
    ...
    if ($_REQUEST['search_term']) {
        ...
        ListOutput($subjects_RET, array('TITLE' => ''._division.''), ''._division.'', ''._divisions.'', $link, array(), array('search' =>false, 'save' =>false));
...
if (clean_param($_REQUEST['modfunc'], PARAM_ALPHAMOD) == 'delete' && AllowEdit()) {
    ...
    if (clean_param($_REQUEST['course_period_id'], PARAM_ALPHANUM)) {
        ...
        if ($course1['COURSE_PERIOD_ID'] != '') {
            ...
            DrawHeaderHome('<font color=red>' ._courseCannotBeDeleted. ' ' . _periodsWereFound . '.</font>');
            echo '<div align=left><a href=Modules.php?modname=schoolsetup/Courses.php&subject_id=' . $subject_id . '&course_id=' . $course_id . ' style="text-decoration:none"><b>'. _returnTo . _courseManager.'</b></a></div>';
            PopTable('footer');
        } else {
            if (DeletePromptCommon($table)) {
                if (BlockDelete($table)) {
        ...
        if ($subject1['COURSE_ID'] != '') {
            PopTable('header', ''._unableToDelete.'');
            DrawHeaderHome('<font color=red>' . _divisionCannotBeDeleted . ' ' . _coursesWereFound .'.</font>');
            echo '<div align=left><a href=Modules.php?modname=schoolsetup/Courses.php&subject_id=' . $subject_id . ' style="text-decoration:none"><b>'. _returnTo . _courseManager . '</b></a></div>';
            PopTable('footer');
        } else {
            //if (DeletePromptCommon($table)) { //original using $table = 'subject'; see line 1576 above
            if (DeletePromptCommon('division')) { //ADDED by Africa's Hope - Evan STewart - 08-12-2021 - change 'subject' to 'division for delete prompt
                if (BlockDelete($table)) {
                    $sql[] = "DELETE FROM course_subjects WHERE SUBJECT_ID='$subject_id'";
                    $courses = DBGet(DBQuery("SELECT COURSE_ID FROM courses WHERE SUBJECT_ID='$subject_id'"));
                    if (count($courses)) {
...
if (!$_REQUEST['modfunc'] && !$_REQUEST['course_modfunc'] && !$_REQUEST['action']) {
    ...
    if (clean_param($_REQUEST['course_period_id'], PARAM_ALPHANUM)) {
        ...
        //ADDED by Africa's Hope - Evan Stewart //add asterisk and text-danger style to label, add required option to input fucntion parameter
        $header .= '<div class="form-group"><label class="col-md-4 control-label text-right">'._shortName.'<span class="text-danger">*</span></label><div class="col-md-8">' . TextInput($RET['SHORT_NAME'], 'tables[course_periods][' . $_REQUEST['course_period_id'] . '][SHORT_NAME]', '', 'required', $div) . '</div></div>';
        ...
        if ($_REQUEST['course_period_id'] == 'new')
            //ADDED by Africa's Hope - Evan Stewart //add asterisk and text-danger style to label, add required option to input fucntion parameter
            $header .= '<div class="form-group"><label class="col-md-4 control-label text-right">'._calendar.'<span class="text-danger">*</span></label><div class="col-md-8">' . SelectInput($RET['CALENDAR_ID'], 'tables[course_periods][' . $_REQUEST['course_period_id'] . '][CALENDAR_ID]', '', $options, 'N/A', ' id=calendar_id onchange=reset_schedule();', $div, 'required') . '</div></div>';
        else {
            ...
            $cal_RET = $cal_RET[1];
            //ADDED by Africa's Hope - Evan Stewart //add asterisk and text-danger style to label, add required option to input fucntion parameter
            $header .= '<div class="form-group"><label class="col-md-4 control-label text-right">'._calendar.'<span class="text-danger">*</span></label><div class="col-md-8">' . SelectInput($cal_RET['CALENDAR_ID'], 'tables[course_periods][' . $_REQUEST['course_period_id'] . '][CALENDAR_ID]', '', $options, 'N/A', ' id=calendar_id onchange=reset_schedule();', $div,'required') . '</div></div>';
        }
        $header .= '</div>'; //.col-sm-6.col-lg-4
        ...
        //ADDED by Africa's Hope - Evan Stewart //add asterisk and text-danger style to label, add required option to input fucntion parameter
            $header .= '<div class="form-group"><label class="col-md-4 control-label text-right">'._primaryTeacher.'<span class="text-danger">*</span></label><div class="col-md-8">' . SelectInputDisabledMsg($RET['TEACHER_ID'], 'tables[course_periods][' . $_REQUEST['course_period_id'] . '][TEACHER_ID]', '', $teachers, 'N/A', '', $div, ""._toChangeTeacherGoToSchoolSetupCoursesTeacherReAssignment."") . '</div></div>';
        else
            $header .= '<div class="form-group"><label class="col-md-4 control-label text-right">'._primaryTeacher.'<span class="text-danger">*</span></label><div class="col-md-8">' . SelectInput($RET['TEACHER_ID'], 'tables[course_periods][' . $_REQUEST['course_period_id'] . '][TEACHER_ID]', '', $teachers, 'N/A', '', $div, 'required') . '</div></div>';
        $header .= '</div>'; //.col-sm-6.col-lg-4
        $header .= '</div>'; //.row
        ...
        /*ORIGINAL $header .= '<div class="form-group"><label class="col-md-4 control-label text-right">'._seats.'</label><div class="col-md-8"><div class="col-md-4">' . TextInput($RET['TOTAL_SEATS'], 'tables[course_periods][' . $_REQUEST['course_period_id'] . '][TOTAL_SEATS]', '', 'size=4 class=form-control', $div) . '</div>';
        if ($_REQUEST['course_period_id'] != 'new')
        ...
        */
        if ($_REQUEST['course_period_id'] != 'new'){
            //ADDED by Africa's Hope - Evan Stewart //add asterisk and text-danger style to label, add required option to input fucntion parameter
            $header .= '<div class="form-group"><label class="col-md-4 control-label text-right">'._seats.'<span class="text-danger">*</span></label><div class="col-md-8"><div class="col-md-4">' . TextInput($RET['TOTAL_SEATS'], 'tables[course_periods][' . $_REQUEST['course_period_id'] . '][TOTAL_SEATS]', '', 'size=4 class=form-control', $div,'required') . '</div>';
            $header .= '<label class="col-md-4 text-success text-right">'._availableSeats.'</label><div class="col-md-4"><span class="seat-count text-success">' . $RET['AVAILABLE_SEATS'] . '</span></div>';
        } else {
            //ADDED by Africa's Hope - Evan Stewart //add asterisk and text-danger style to label, add required option to input fucntion parameter
            $header .= '<div class="form-group"><label class="col-md-4 control-label text-right">'._seats.'<span class="text-danger">*</span></label><div class="col-md-8"><div class="col-md-4">' . TextInput('25', 'tables[course_periods][' . $_REQUEST['course_period_id'] . '][TOTAL_SEATS]', '', 'size=4 class=form-control', $div,'', 'required') . '</div>';
        }
        ...
        $header .= '<div class="col-sm-6 col-lg-4">';
        //ADDED by Africa's Hope - Evan Stewart //add asterisk and text-danger style to label, add required option to input fucntion parameter
        $header .= '<div class="form-group"><label class="col-md-4 control-label text-right">'._gradingScale.'<span class="text-danger">*</span></label><div class="col-md-8">' . SelectInput($RET['GRADE_SCALE_ID'], 'tables[course_periods][' . $_REQUEST['course_period_id'] . '][GRADE_SCALE_ID]', '', $options, ''._notGraded.'', 'onChange="toggle_course_weight(this,' . $cp_id_js . ');"', $div,'required') . '</div></div>';
        $header .= '</div>'; //.col-sm-6.col-lg-4
        $header .= '</div>'; //.row
        ...
        $header .= '<div class="col-sm-6 col-lg-4">';
        //ADDED by Africa's Hope - Evan Stewart //add asterisk and text-danger style to label, add required option to input fucntion parameter
        $header .= '<div class="form-group"><label class="col-md-4 control-label text-right">'._creditHours.'<span class="text-danger">*</span></label><div class="col-md-8">' . TextInput(sprintf('%0.3f', $RET['CREDITS']), 'tables[course_periods][' . $_REQUEST['course_period_id'] . '][CREDITS]', '', 'size=4 required') . '</div></div>';
        $header .= '</div>'; //.col-sm-6.col-lg-4
        //ADDED by Africa's Hope - Evan Stewart
        //Hide Gender Restriction selection box.  All classes Africa AG classes will be gender open
        /*
        $header .= '<div class="col-sm-6 col-lg-4">';
        ...
        */
        ...
        //      --------------------------------------------- Temp Coment -------------------------------------------------     //
        //ADDED by Africa's Hope - Evan Stewart
        //hide Parent Period form field.  OASIS will not use Parent log in options
        /*
        if ($_REQUEST['course_period_id'] != 'new' && $RET['PARENT_ID'] != $_REQUEST['course_period_id']) {
...
//        echo $RET['DOES_BREAKOFF'];
        ...
        //ADDED by Africa's Hope - Evan Stewart
        //default Affects Class Rank to VALUE=Y for new course period, use saved option for saved course period. THis will cause new course periods to have Affects Class Rank enabled by default
        ...
        if ($_REQUEST['course_period_id'] != 'new') {
            ...
            $header .= CheckboxInputSwitch('Y', 'tables[course_periods][' . $_REQUEST['course_period_id'] . '][DOES_CLASS_RANK]', _affectsClassRank, $checked, $new, '<i class="icon-checkbox-checked"></i>', '<i class="icon-checkbox-unchecked"></i>', $div, ' switch-success');
        }
        //END Africa's Hope addition
        ...
        $header .= '<DIV id=date_range style=display:' . ($RET['BEGIN_DATE'] ? 'block' : 'none') . ' class="clearfix"><div class="col-md-4"><div class="form-group"><label class="control-label text-right col-md-2">'._begins.' <span class="dateTip">'._dateFormatTip.'</span></label><div class="col-md-10">' . DateInputAY($RET['BEGIN_DATE'], 'begin', 1, ($_REQUEST['conflict'] == 'y' ? true : false), '') . '</div></div></div><div class="col-md-4"><div class="form-group"><label class="control-label text-right col-md-2">'._ends.' <span class="dateTip">'._dateFormatTip.'</span></label><div class="col-md-10">' . DateInputAY($RET['END_DATE'], 'end', 2, ($_REQUEST['conflict'] == 'y' ? true : false), '') . '</div></div></div></DIV>';
        ...
        if ($_REQUEST['course_period_id'] == 'new' && $not_pass == false) {
            ...
            foreach ($caldays as $day => $short_day) {
                ...
                //ADDED by Africa's Hope - Evan Stewart
                //Show the Short Day abbreviation on "Meeting Days" check box list
                //$header .= CheckboxInput($value, 'tables[course_period_var][' . $_REQUEST['course_period_id'] . '][DAYS][' . $day . ']', ($day == 'U' ? 'U' : $day), $checked, true, '', '', false);
                $header .= CheckboxInput($value, 'tables[course_period_var][' . $_REQUEST['course_period_id'] . '][DAYS][' . $day . ']', $short_day, $checked, true, '', '', false);
                //END Africa's Hope addition
            }
            if ($not_pass != true)
        ...
        $header .= '<div class="form-group"><label class="col-md-4 control-label text-right">'._title.' :<span class="text-danger">*</span></label><div class="col-md-8">' . TextInput($RET['TITLE'], 'tables[courses][' . $_REQUEST['course_id'] . '][TITLE]', '', 'id=course_title class=cell_mod_wide required') . '</div></div>';
        $header .= '</div>'; //.col-md-8
        $header .= '<div class="col-lg-4">';
        //ADDED by Africa's Hope - Evan Stewart  //add text-danger and * to label, add 'required' to TextInput options parameter
        $header .= '<div class="form-group"><label class="col-md-4 control-label text-right">'._shortName.' :<span class="text-danger">*</span></label><div class="col-md-8">' . TextInput($RET['SHORT_NAME'], 'tables[courses][' . $_REQUEST['course_id'] . '][SHORT_NAME]', '', 'id=short_name class=form-control required') . '</div></div>';
        $header .= '</div>'; //.col-md-4
        $header .= '</div>'; //.row
        ...
        $header .= '<div class="form-group"><label class="col-md-4 control-label text-right">'._gradeLevel.' :<span class="text-danger">*</span></label><div class="col-md-8">' . SelectInput($RET['GRADE_LEVEL'], 'tables[courses][' . $_REQUEST['course_id'] . '][GRADE_LEVEL]', '', $grade_levels,'','','','required') . '</div></div>';
        $header .= '</div>'; //.col-md-4
        $header .= '<div class="col-lg-4">';
        foreach ($subjects_RET as $type)
            $options[$type['SUBJECT_ID']] = $type['TITLE'];

        $header .= '<div class="form-group"><label class="col-md-4 control-label text-right">'._division.' :</label><div class="col-md-8">' . SelectInput($RET['SUBJECT_ID'] ? $RET['SUBJECT_ID'] : $_REQUEST['subject_id'], 'tables[courses][' . $_REQUEST['course_id'] . '][SUBJECT_ID]', '', $options, false) . '</div></div>';
        $header .= '</div>'; //.col-md-4
        $header .= '</div>'; //.row
    ...
    if (clean_param($_REQUEST['subject_id'], PARAM_ALPHANUM) && $_REQUEST['subject_id'] != 'new') {
        ...
        $sql = "SELECT COURSE_ID,c.TITLE, CONCAT_WS(' - ',c.short_name,c.title) AS GRADE_COURSE FROM courses c LEFT JOIN school_gradelevels sg ON c.grade_level=sg.id WHERE SUBJECT_ID='$_REQUEST[subject_id]' ORDER BY c.SHORT_NAME";
        $QI = DBQuery($sql);
